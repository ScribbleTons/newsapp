...
const RegisterScreen: FC<RegisterProps> = ({navigation}) => {
	...
	try{
		...
		addUser(payload).then((res) => {
			if(res.isExist){
				Alert.alert("User already exist in the system, please login.");
				setLoading(false)
				return;
			}
			setLoading(false)
    ...
    <View style={[styles.container, {backgroundColor: colors.accent}]}>
      <View style={{display:"flex"}}>
        ...
        <View style={{marginHorizontal:15, display:"flex" }}>
          <Headline style={{color:colors.text, marginBottom:2, marginTop:4, fontSize:14, textAlign:"center"}}>
            Register to continue or <Text style={{color:"blue", textDecorationLine:"underline"}} onPress={()=>navigation.navigate("Login")}>log in</Text>
          </Headline>
	  	<TextInput 
		mode="outlined" 
		label="Email" 
